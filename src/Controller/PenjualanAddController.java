...
public class PenjualanAddController implements Initializable {
    ...
    @FXML
    private TextField flJumPro;

    @FXML
    private TextField flNamPro;

    @FXML
    private TextField flTotHar;
   ...
   public void showTotalHarga(){
       ...
       try{
           ...
           lbTotalHarga.setText(Double.toString(count));
       }
       catch(Exception ex){
           ex.printStackTrace();
       }           
   ...
   public void showJumlahProduk(){
       int count = 0;
       String show = "SELECT jumlahProduk FROM penjualan";
       Connection CONN = getConnection();
       Statement st;
       ResultSet rs; 
       ...
       try{
           ...
           rs = st.executeQuery(show);
           while(rs.next()){
               count+= rs.getInt("jumlahProduk");
           }     
           lbTotalProduk.setText(Integer.toString(count));
       }
       catch(Exception ex){
           ex.printStackTrace();
       }           
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cmDataPenjualan.setOnAction(event ->{
            ...
            if (selected.equals("Tambah")) {
                try {
                    Penjualan peju = new Penjualan(flNamPro.getText(),Integer.parseInt(flJumPro.getText()),Double.parseDouble(flTotHar.getText()));
                    pj.addPenjualan(peju);
